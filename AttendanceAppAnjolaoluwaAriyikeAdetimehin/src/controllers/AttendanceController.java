...
public class AttendanceController {
    ...
    private void listSwipesByCardIdOrderedByDateTime() {        
        ...
        //converted the response to lowercase to avoid issues of case difference
    ...
    private void listSwipeStatistics() {
        ...
        Collections.sort(temp, Swipe.SwipeComparator);
        int counter = 0;
        for (Swipe swipes : temp) {
            counter++;
        }
        
        Calendar calendarTemp = temp.get(temp.size()-1).getSwipeDateTime();
        System.out.println("Total number of swipes for "+ response + " is "+ counter +
                ", and time of most recent swipe is "+ formatSwipeDateTime(calendarTemp));
    }
    
    private  void saveToFile(){
        System.out.format("\033[31m%s\033[0m%n", "Save To File");
        System.out.format("\033[31m%s\033[0m%n", "============"); 
        
                    InputHelper inputHelper = new InputHelper();
                    String response = inputHelper.readString("Saving changes. Please enter file name");                
                    repository.store(response);
  
        if (response.isEmpty()){
            System.out.println("All changes made will be discarded");
    }
}
}
